...
// Fetch filters data
const fetchFilters = async (): Promise<FiltersData> => {
    const { data } = await axios.get('http://localhost:8000/api/v1/filters');
    return {
        categories: data.categories,
        brands: data.brands,
        minPrice: 100, // Replace with actual data
        maxPrice: 5000, // Replace with actual data
        transmissions: ['manual', 'automatic'], // Replace with actual data
        fuelTypes: ['gasoline', 'diesel'], // Replace with actual data
    };
};
...
const FilterComponent = () => {
    ...
    // Update filters state when any filter changes
    useEffect(() => {
    ...
    }, [selectedCategories, selectedBrands, selectedTransmission, priceRange, selectedFuelType, setFilters]);
    ...
    return (
        <div className="w-full p-4 bg-gray-50 border rounded-md">
            ...
            <button className="mt-4 bg-blue-500 text-white px-4 py-2 rounded">Reset Filters</button>
        </div>
    );
};
